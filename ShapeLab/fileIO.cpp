...
#include <direct.h>   // 用于创建目录（Windows 平台）
...
void fileIO::outputISOSurfaceMesh(
	...
	for (GLKPOSITION posMesh = isosurfaceSet->GetMeshList().GetHeadPosition(); posMesh != nullptr;) {
		...
		if (!singleOutputMode && isoSurface->GetIndexNo() < maxLayerNum) {
			...
			std::string path = "../Model/IsoSurface/" + modelName + "/";
			this->_outputSingleISOSurface(offMode, isoSurface, path, false, false);
			outputlayerheight(isoSurface,modelName);
			continue;
		}
...
void fileIO::outputlayerheight(QMeshPatch* initialModel,std::string modelName)
{
	/*std::string path = "../Model/IsoSurface/" + modelName + "/";
	path += "layerheight/";
	path += to_string(initialModel->GetIndexNo() * 100);
	path += ".txt";
	ofstream nodeheight(path);

	for (GLKPOSITION posNode = initialModel->GetNodeList().GetHeadPosition(); posNode != nullptr;) {
		QMeshNode* node = (QMeshNode*)initialModel->GetNodeList().GetNext(posNode);
		nodeheight << node->layerheight<< endl;
	}
	nodeheight.close();*/
	// 构造路径
	string basePath = "../Model/IsoSurface/" + modelName + "/";
	string layerHeightPath = basePath + "layerheight/";
	string filePath = layerHeightPath + to_string(initialModel->GetIndexNo() * 100) + ".txt";

	// 确保目录存在并递归创建
	size_t pos = 0;
	do {
		pos = layerHeightPath.find_first_of("/\\", pos + 1); // 找到下一个分隔符
		string subPath = layerHeightPath.substr(0, pos);

		// check the directory exist
		struct stat info;
		if (stat(subPath.c_str(), &info) != 0) { //  
#ifdef _WIN32
			if (_mkdir(subPath.c_str()) != 0) { // Windows use _mkdir
#else
			if (mkdir(subPath.c_str(), 0777) != 0) { // POSIX use mkdir
#endif
				cerr << "cannot creat dir：" << subPath << endl;
				return;
			}
			}
		else if (!(info.st_mode & S_IFDIR)) { //  
			cerr << "path exist but not dir：" << subPath << endl;
			return;
		}
		} while (pos != string::npos);
	//cout << "dir exists：" << layerHeightPath << endl;

 	ofstream nodeheight(filePath);
	if (!nodeheight.is_open()) {
		cerr << "cannot open path：" << filePath << endl;
		return;
	}
...
void fileIO::searchInnerline(QMeshPatch* layer, double scalarvalue) {


	//heatMethodField* heatFieldOperator = new heatMethodField(layer);
	...
	//	if (Node->isouterline) { Node->geoFieldValue = 1.0; Node->selected = true; }
	//}
	...
	//	QMeshNode* startnode; QMeshNode* endnode;
	//	startnode = edge->GetStartPoint();
	//	endnode = edge->GetEndPoint();
	//	if ((startnode->geoFieldValue - scalarvalue) * (endnode->geoFieldValue - scalarvalue)<0) {
	//		QMeshNode* node = new QMeshNode;
	//		Eigen::Vector3d ps, pe, pp, nl, nr, nn;
	//		double alpha = (scalarvalue - startnode->geoFieldValue) / (endnode->geoFieldValue - startnode->geoFieldValue);
	//		startnode->GetCoord3D(ps);
	//		endnode->GetCoord3D(pe);
	//		edge->GetLeftFace()->GetNormal(nl[0], nl[1], nl[2]);
	//		edge->GetRightFace()->GetNormal(nr[0], nr[1], nr[2]);
	//		for (int i = 0; i < 3; i++)
	//		{
	//			pp[i] = (1 - alpha) * ps[i] + alpha * pe[i];
	//			nn[i] = (1 - alpha) * nr[i] + alpha * nl[i];
	//		}
	//		node->SetCoord3D(pp);
	//		node->SetNormal(nn(0), nn(1), nn(2));
	//		node->layerheight = startnode->layerheight * (1 - alpha) + startnode->layerheight * alpha;
	//		node->relatedEdgeIndex = edge->GetIndexNo();
	//		node->SetIndexNo(layer->ExtraInnernodelist.GetCount());
	//		layer->ExtraInnernodelist.AddTail(node);
	//	}
	...
	//for (GLKPOSITION Pos = layer->GetNodeList().GetHeadPosition(); Pos;) {
	//	QMeshNode* Node = (QMeshNode*)layer->GetNodeList().GetNext(Pos);
	//	if (Node->geoFieldValue < scalarvalue) {
	//		Node->ininnerline = true;
	//		cout << "innerline" << endl;
	//	}
	//}
}
...
void fileIO::inputInstalledIsoSurface(
	PolygenMesh* isosurfaceSet, std::vector<std::string>& layersFiles, std::vector<std::string>& LheightFiles, std::string folderPath) {
	
	std::cout << folderPath << std::endl;

	if (layersFiles.size() != LheightFiles.size()) printf("fileIO - SYSTEM FILE reading ERROR!\n");
	...
	for (int i = 0; i < layersFiles.size(); i++) {
		...
		layer->SetIndexNo(i);
		if (LheightFiles.size() != 0)
		{
			char Lheightfilename[1024]; sprintf(Lheightfilename, "%s%s%s", folderPath.data(), "/layerheight/", LheightFiles[i].data());
			std::cout << Lheightfilename << std::endl;
			FILE* fp = fopen(Lheightfilename, "r");
			if (!fp) { printf("Can not open the field data file\n"); return; }

			Eigen::MatrixXd Lheight = Eigen::MatrixXd::Zero(layer->GetNodeNumber(), 1);

			for (int i = 0; i < layer->GetNodeNumber(); i++)
			{
				fscanf(fp, "%lf\n", &Lheight(i, 0));
			...
			for (GLKPOSITION Pos = layer->GetNodeList().GetHeadPosition(); Pos;) {
				QMeshNode* node = (QMeshNode*)layer->GetNodeList().GetNext(Pos);
				node->layerheight = Lheight(index, 0);
				//cout << "layheiht" << node->layerheight;
				index++;
			...
			//Startnode->layerheight = 0;
			Startnode->isouterline = true;

			QMeshNode* Initialnode = Startnode;
			QMeshEdge* Prevedge = NULL;

			do
			{
				for (GLKPOSITION Pos = Startnode->GetEdgeList().GetHeadPosition(); Pos;) {
					QMeshEdge* edge = (QMeshEdge*)Startnode->GetEdgeList().GetNext(Pos);
					if (edge != Prevedge)
						if (edge->GetLeftFace() == NULL || edge->GetRightFace() == NULL)
						{
							QMeshNode* Nextnode = edge->GetStartPoint();
							if (Nextnode == Startnode)
							{
								Nextnode = edge->GetEndPoint();
							}
							Startnode = Nextnode;
							//Nextnode->layerheight = 0;
							Nextnode->isouterline = true;
							Prevedge = edge;
							break;
						}
				}
			} while (Initialnode != Startnode);

			QMeshEdge* PrevInneredge = NULL;
			for (GLKPOSITION Pos = layer->GetEdgeList().GetHeadPosition(); Pos;) {
				QMeshEdge* edge = (QMeshEdge*)layer->GetEdgeList().GetNext(Pos);
				if (edge->GetLeftFace() == NULL || edge->GetRightFace() == NULL)
				{
					if (edge->GetStartPoint()->isouterline == false && edge->GetEndPoint()->isouterline == false)
					{
						PrevInneredge = edge; break;
					}		
				}
			}
			if (PrevInneredge==NULL)
			{
				layer->drawlayerheight = true;
				layer->drawThisIsoLayer = true;
				cout << "search visual innerloop" << endl;
				//searchInnerline(layer,0.002);
				continue;
			}
			QMeshNode* InitialInnerNode = PrevInneredge->GetStartPoint();
			QMeshNode* StartInnerNode = PrevInneredge->GetStartPoint();
			StartInnerNode->isinnerline = true;
		//	layer->ExtraInnernodelist.AddTail(StartInnerNode);
			do
			{
				for (GLKPOSITION Pos = StartInnerNode->GetEdgeList().GetHeadPosition(); Pos;) {
					QMeshEdge* edge = (QMeshEdge*)StartInnerNode->GetEdgeList().GetNext(Pos);
					if (edge != PrevInneredge)
						if (edge->GetLeftFace() == NULL || edge->GetRightFace() == NULL)
						{
							QMeshNode* Nextnode = edge->GetStartPoint();
							if (Nextnode == StartInnerNode)
							{
								Nextnode = edge->GetEndPoint();
							}
							StartInnerNode = Nextnode;
							Nextnode->isinnerline = true;
							//layer->ExtraInnernodelist.AddTail(Nextnode);
							PrevInneredge = edge;
							break;
						}
				}
			} while (InitialInnerNode != StartInnerNode);
		layer->drawlayerheight = true;
		}
		else
		{
			int index = 0;
			double mazz = -INFINITY;
			double xx, yy, zz;
			QMeshNode* Startnode;
			for (GLKPOSITION Pos = layer->GetNodeList().GetHeadPosition(); Pos;) {
				QMeshNode* node = (QMeshNode*)layer->GetNodeList().GetNext(Pos);
				node->layerheight =1.1;
				index++;
				node->GetCoord3D(xx, yy, zz);
				if (zz > mazz) {
					mazz = zz;
			...
			if (PrevInneredge == NULL)
			{
				layer->drawlayerheight = true;
				layer->drawThisIsoLayer = true;
				cout << "no inner loop" << endl;
			//	searchInnerline(layer, 0.01);
				continue;
			}
			QMeshNode* InitialInnerNode = PrevInneredge->GetStartPoint();
			QMeshNode* StartInnerNode = PrevInneredge->GetStartPoint();
		...
		for (GLKPOSITION Pos = layer->GetEdgeList().GetHeadPosition(); Pos;) {
			...
			if (edge->GetLeftFace() == NULL || edge->GetRightFace() == NULL)
			{
				if (!edge->GetStartPoint()->isinnerline&& !edge->GetStartPoint()->isouterline)
				{
					cout << "more than one innerbondary!" << endl;
				}
				if (!edge->GetEndPoint()->isinnerline && !edge->GetEndPoint()->isouterline)
				{
					cout << "more than one innerbondary!" << endl;
				}
			}
	...
	for (GLKPOSITION Pos = isosurfaceSet->GetMeshList().GetHeadPosition(); Pos;) {
		...
		for (GLKPOSITION nodePos = layer->GetNodeList().GetHeadPosition(); nodePos;) {
			...
			if (node->layerheight < minheight) {
				minheight = node->layerheight;
			}
			if (node->layerheight > maxheight) {
				maxheight = node->layerheight;
			}
		}
	}
...
void fileIO::_outputSingleISOSurface(
	...
	if (isoSurface->includeSupportRegion) 
		...
		//---- Output vector field of iso-surface for futher zigzag tool-path generation
		fieldPath += "field.txt";
