...
void IsoLayerGeneration::generateIsoSurface_adaptiveDistanceControl(
	...
	double startisovalue=0;

	/* first Layer */

	if (isoSurface->GetMeshList().GetCount() == 0)
	{
		double initLayerISOValue = 0.001; // initial layer iso value
		startisovalue = initLayerISOValue;
		layer = _generatesingleIsoSurface(startisovalue, isoSurface);
	...
	else
	{
		...
		startisovalue = bLayer->isoSurfaceValue;
	}
	...
	do
	{
		...
		double layeriso = layer->isoSurfaceValue;
		this->_insertLayerToSurfaceSet_smooth(layer, 10, isoSurface, bLayer, false); // always smooth layer before check the layer hight - avoid numerical error
	...
	do {
		...
		for (GLKPOSITION posMesh = isoSurface->GetMeshList().GetHeadPosition(); posMesh != nullptr;) {
			QMeshPatch* layer = (QMeshPatch*)isoSurface->GetMeshList().GetNext(posMesh);
			if (layer->isoSurfaceValue<= startisovalue)
			{
				continue;
			}
	...
	for (GLKPOSITION posMesh = isoSurface->GetMeshList().GetHeadPosition(); posMesh != nullptr;) {
		...
		i++;
		//std::cout << "layer node nume=" << layer->GetNodeNumber() << std::endl;
	}
	_installLayerDis_toNode(isoSurface);
}
void IsoLayerGeneration::_installLayerDis_toNode(QMeshPatch* lastlayer, QMeshPatch* thislayer)
{
	int index = 0;
	/* build PQP model */
	PQP_Model* pqpModel = new PQP_Model();
	pqpModel->BeginModel();  PQP_REAL p1[3], p2[3], p3[3];
	for (GLKPOSITION Pos = lastlayer->GetFaceList().GetHeadPosition(); Pos;) {
		QMeshFace* Face = (QMeshFace*)lastlayer->GetFaceList().GetNext(Pos);
	...
	pqpModel->EndModel();
	this->_checkLayerDistance(lastlayer, thislayer, pqpModel);
	std::cout << "finish install layer distance" << std::endl;
}

void IsoLayerGeneration::_installLayerDis_toNode(PolygenMesh* isosurfaceset)
{
	int layerindex = 0;
	for (GLKPOSITION posMesh = isosurfaceset->GetMeshList().GetHeadPosition(); posMesh != nullptr;) {
		QMeshPatch* layer = (QMeshPatch*)isosurfaceset->GetMeshList().GetAt(posMesh);
		layerindex = layer->GetIndexNo();
		//if (layerindex ==0) { std::cout << "start" << std::endl; continue; }
		GLKPOSITION prevposMesh;
		if (posMesh->prev==nullptr)
		{
		...
		{
			if (((QMeshPatch*)isosurfaceset->GetMeshList().GetAt(posMesh->prev))->GetFaceNumber() < 20) {
			...
			prevposMesh = posMesh->prev;//this may cause nullptr!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		}
		posMesh = posMesh->next;
		QMeshPatch* prevlayer;
		int k = 0;
		do
		{	
		//	prevposMesh = prevposMesh->prev;
			prevlayer = (QMeshPatch*)isosurfaceset->GetMeshList().GetAt(prevposMesh);
			int index = 0;
			PQP_Model* pqpModel = new PQP_Model();
			pqpModel->BeginModel();  PQP_REAL p1[3], p2[3], p3[3];
			for (GLKPOSITION Pos = prevlayer->GetFaceList().GetHeadPosition(); Pos;) {
				QMeshFace* Face = (QMeshFace*)prevlayer->GetFaceList().GetNext(Pos);
			...
			for (GLKPOSITION Pos = layer->GetNodeList().GetHeadPosition(); Pos;) {
				...
				Eigen::Vector3d ves,sumvec;
				sumvec.setZero();
				double sumarea=0;
				for (int i = 0; i < 3; i++)
				{
					ves(i) = dres.P1()[i] - dres.P2()[i];
				}
				for (GLKPOSITION  facepos=node->GetFaceList().GetHeadPosition();facepos;)
				{
					QMeshFace* face = (QMeshFace*)node->GetFaceList().GetNext(facepos);
					Eigen::Vector3d nvec;
					face->GetNormal(nvec(0), nvec(1), nvec(2));
					sumvec = sumvec + face->CalArea() * nvec;
					sumarea += face->CalArea();
				}
				sumvec = sumvec / sumarea;
				minDist =fabs( minDist * (ves.dot(sumvec) / (ves.norm() * sumvec.norm())));
				node->heightvector.push_back(minDist);
			}
			k ++ ;
		} while (prevlayer->istrimed==true&&k<=15);
		//std::cout << "layerindex=" << layer->GetIndexNo() << std::endl;
		for (GLKPOSITION Pos = layer->GetNodeList().GetHeadPosition(); Pos;) {
			QMeshNode* node = (QMeshNode*)layer->GetNodeList().GetNext(Pos);
			node->layerheight =* std::min_element(node->heightvector.begin(), node->heightvector.end());
		//	std::cout << "size of vec=" << node->heightvector.size() << "height=" << node->layerheight << std::endl;
			if (isosurfaceset->maxlayerheight <= node->layerheight)
			{
				isosurfaceset->maxlayerheight = node->layerheight;
			}
			if (isosurfaceset->minlayerheight >=node->layerheight)
			{
				//if (node->layerheight > 1.1) {
				if(layerindex>=1.0)
					isosurfaceset->minlayerheight = node->layerheight;
			//	}
				/*else
				{
					node->layerheight = 1.1;
...
double IsoLayerGeneration::_detectNextLayerISOValue(QMeshPatch* bLayer, double minLayerHight) {
	...
	double higherBound_isoValue = 0.999;
...
void IsoLayerGeneration::_checkLayerDistance(QMeshPatch* bLayer, QMeshPatch* tLayer, PQP_Model* pqpModel) {
	/* compute distance for each layer and get result */
	double xx, yy, zz;
	for (GLKPOSITION Pos = tLayer->GetFaceList().GetHeadPosition(); Pos;) {
		QMeshNode* node = (QMeshNode*)tLayer->GetFaceList().GetNext(Pos);
		PQP_DistanceResult dres; dres.last_tri = pqpModel->last_tri;
		PQP_REAL p[3]; 
		node->GetCoord3D(xx, yy, zz);
		p[0] = xx; p[1] = yy; p[2] = zz;
...
void IsoLayerGeneration::_checkNodeSet_withinBlayerRegion(QMeshPatch* bLayer, 
	Eigen::MatrixXd& nodePosSet, double maxLayerHight, Eigen::VectorXi& deleteNodeIndexSet){//errrrrrrrrrrrrrrrrrrrrrrrrr
...
void IsoLayerGeneration::_triming_workingSurface(QMeshPatch* isoLayer) {
	...
	for (GLKPOSITION Pos = isoLayer->GetEdgeList().GetHeadPosition(); Pos;) {
		...
		double a,b;
		if (Edge->GetStartPoint()==nullptr)
		{
			std::cout << "starterr " << std::endl;
		}
		else
		{
			a= Edge->GetStartPoint()->layerThickDistance;
		}
		if (Edge->GetEndPoint() == nullptr)
		{
			std::cout << "enderr " << std::endl;
		}
		else
		{
			b = Edge->GetEndPoint()->layerThickDistance;
		}
	...
	for (GLKPOSITION Pos = isoLayer->GetEdgeList().GetHeadPosition(); Pos;) {
		...
		if (Edge->isLocate_CutNode_layerEdge) {
			...
			double xx, yy, zz;
			Edge->installed_CutNode->GetCoord3D(xx, yy, zz);//installend_cutnode has no coord3d
