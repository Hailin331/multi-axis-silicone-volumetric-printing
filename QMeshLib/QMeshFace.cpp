...
void QMeshFace::CalPlaneEquation(double& xxx, double& yyy, double& zzz, double& ddd)
{
	GLKGeometry geo;
	double p[3][3], nv[3], cp[3];	int i;
	double aa, bb, cc, dd;
	...
	dd = aa * aa + bb * bb + cc * cc;	if (dd < 1.0e-10) dd = 1.0;
	dd = sqrt(dd);
	xxx = aa / dd;	yyy = bb / dd;	zzz = cc / dd;	//abcd[3]=-(cp[0]*aa+cp[1]*bb+cp[2]*cc);
	ddd = -(cp[0] * abcd[0] + cp[1] * abcd[1] + cp[2] * abcd[2]);
	//std::cout << xxx << "," << yyy << "," << zzz << "," << ddd << std::endl;
}
