...
void QMeshPatch::ComputeBoundingBox(double boundingBox[])
{
    ...
    for(Pos=nodeList.GetHeadPosition();Pos!=NULL;) {
        ...
        if (xx<boundingBox[0]) boundingBox[0]=xx; // xmin
        if (xx>boundingBox[1]) boundingBox[1]=xx; // xmax
        if (yy<boundingBox[2]) boundingBox[2]=yy; // ymin
...
void QMeshPatch::expandSelection_fix_handle(int neighborSearchTime) {

	for (int iter = 0; iter < neighborSearchTime; iter++) {
		std::vector<QMeshNode*> fixSet;
		std::vector<QMeshNode*> handleSet;
		for (GLKPOSITION Pos = this->GetNodeList().GetHeadPosition(); Pos;) {
			...
			if (node->isFixed) {
				for (GLKPOSITION Pos = node->GetEdgeList().GetHeadPosition(); Pos;) {
					...
					fixSet.push_back(edge->GetStartPoint());
					fixSet.push_back(edge->GetEndPoint());
				}
			}
			if (node->isHandle) {
				for (GLKPOSITION Pos = node->GetEdgeList().GetHeadPosition(); Pos;) {
					QMeshEdge* edge = (QMeshEdge*)node->GetEdgeList().GetNext(Pos);
					handleSet.push_back(edge->GetStartPoint());
					handleSet.push_back(edge->GetEndPoint());
				}
			}
		}
		for (int i = 0; i < fixSet.size(); i++)fixSet[i]->isFixed = true;
		for (int i = 0; i < handleSet.size(); i++)handleSet[i]->isHandle = true;
	}
