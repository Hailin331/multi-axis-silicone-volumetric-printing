...
QMeshNode::QMeshNode()
{
	...
	//hvector= Eigen::VectorXd::Zero(4);
	//heightvector=new std::vector<double>;
	heightvector.reserve(6);
}
QMeshNode::QMeshNode(QMeshNode*orinode)
{
	indexno = 0;		m_trackingFace = nullptr;
	m_meanCurvatureNormalVector[0] = 0.0;
	m_meanCurvatureNormalVector[1] = 0.0;
	...
	orinode->GetCoord3D(xx, yy, zz);
	orinode->GetNormal(nx, ny, nz);
	SetCoord3D(xx, yy, zz);
	SetNormal(nx, ny, nz);
	layerheight = orinode->layerheight;
	relatedEdgeIndex = orinode->relatedEdgeIndex;
	DisToBoundary = orinode->DisToBoundary;
	DisToInnerBoundary = orinode->DisToInnerBoundary;
}
QMeshNode::~QMeshNode()
{
	faceList.RemoveAll();
...
void QMeshNode::GetCoord3D(Eigen::Vector3d & pp)
{
	pp(0) = coord3D[0];	pp(1) = coord3D[1];	pp(2) = coord3D[2];
}
...
void QMeshNode::SetCoord3D(Eigen::Vector3d& pp)
{
	coord3D[0] = pp(0);	coord3D[1] = pp(1);	coord3D[2] = pp(2);
}
