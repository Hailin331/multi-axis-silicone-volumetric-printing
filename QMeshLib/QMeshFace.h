...
enum cuttype { KEEP,CUT_1,CUT_2, DISCARD};
...
public:
    ...
    bool IsMoreWeight;
    bool visible;
	bool nodefineregion = false;
	bool handleface = false;
	bool selectface = false;
	bool fixedface = false;
	bool containOneBoundEdge = false;
	bool containTwoBoundEdge = false;
	bool errorface = false;
    int identifiedIndex;
	GLKObList attachedList;	// a list of attached object

	int VDCornerCase = -1; 
	// VDCornerCase = 0 --- corner node directly inside the triangle
	// VDCornerCase = 1 --- corner node on another triangle
	// VDCornerCase = 2 --- corner node inside the triangle, but not computed inside
	QMeshFace* connectedVDCornerFace = nullptr;
	cuttype faceType;
	/**************************************/
	bool tetInnerFace = false;
	bool isCollisionOutput = false;
	int splitIndex;
	bool isNIEface = false;
	bool ischecked = false;
	Eigen::Matrix<double,2,3> planenodpos;
	Eigen::Matrix3d Twodirvec;
	bool stripcheck = false;
private:
	int indexno;
	bool flags[8];
...
public:
	...
	//for volume mesh
	QMeshTetra* pLeftTetra = NULL;
	QMeshTetra* pRightTetra = NULL;

	/* below are the parameter added for CVT computing*/
public:
	int getNodePtrNumber(QMeshNode* node);
	bool isBoundaryFace();
	std::vector<QMeshNode*> seedNodeSet;
	bool voronoiRebuild = false;
	bool isIntersetwithPlane = false;

	Eigen::Vector3d fieldVec = Eigen::Vector3d::Zero();

	QMeshEdge* installedIsoEdge = nullptr;
	bool isLocatedIsoEdge;

	Eigen::Vector3d principleStressDir = Eigen::Vector3d::Zero();
	Eigen::Vector3d principleStressDirOld = Eigen::Vector3d::Zero();

	bool isCriticalFace = false;
	double principleStress = 0.00000001;
	double principleStressColor[3] = { 0 };

	//kaguya
	bool secondCaseOffset = false;
	bool secondCaseOffsetInit = false;

	int initCellIndex = -1;
	QMeshNode* VDBoundOffsetCorner = nullptr;
	
	bool isCCFCut = false;
	bool isCCFCutCenter = false;
