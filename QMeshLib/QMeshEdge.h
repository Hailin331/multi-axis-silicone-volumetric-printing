...
public:
    ...
    double weight;
	/********************************************/
	bool intersectNodeIndex = false;
	QMeshNode* installed_CutNode;
	bool isLocate_CutNode_layerEdge=false;
	bool hasoneboundary=false;
	bool isinnerboundary = false;
	bool isoutterboundary = false;
	bool unseeedge = false;
private:
	int indexno;
	bool flags[8];
...
public:
	QMeshNode* voronoiNode;
	bool isVoronoiNodeBuild = false;
	int refineNodeIndex = -1;
	bool isConnectEdge = false;

	QMeshFace* connectedFace = nullptr;

	bool isLocateIsoNode;
	...
	//kaguya
	QMeshNode* VDNode = nullptr;
	std::vector<QMeshNode*> VDOffsetNodeSet;
	std::vector<QMeshNode*> VDoffsetBoundNodeSet;

	bool isCCFInitEdge = false;
	bool CCFSplitEdge = false;

	int initCellIndex = -1; // initial cell boundary index = -1;
	bool isOffsetDirLeft;

	QMeshEdge* leftOffsetEdge = nullptr;
	QMeshEdge* rightOffsetEdge = nullptr;

	bool VDTopoConnected = false;
	int VDEdgeIndex = -1;
	bool VDBoundEdge = false;
	bool invisible = false;
	bool stripecheck = false;
	QMeshNode* sCutNode; // cutting node close to start point
	QMeshNode* eCutNode; // cutting node close to end point
