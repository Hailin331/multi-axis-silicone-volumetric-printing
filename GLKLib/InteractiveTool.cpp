...
InteractiveTool::InteractiveTool(GLKLib *pGLKLib, QMeshPatch *meshPatch_,  GLKMouseTool *mouseTool, select_type selectType, bool Select,bool xpos,bool xneg,bool ypos,bool yneg,bool zpos,bool zneg,bool dis)
{
    pGLK = pGLKLib;
...
InteractiveTool::InteractiveTool(GLKLib *pGLKLib, GLKObList *polygenMeshList,  GLKMouseTool *mouseTool, select_type selectType, bool Select, bool xpos, bool xneg, bool ypos, bool yneg, bool zpos, bool zneg,bool dis)
{
    pGLK = pGLKLib;
...
InteractiveTool::InteractiveTool(GLKLib* pGLKLib, GLKObList* polygenMeshList, GLKMouseTool* mouseTool, select_type selectType, bool Select, bool xpos, bool xneg, bool ypos, bool yneg, bool zpos, bool zneg, bool dis,int handleindex)
{
	pGLK = pGLKLib;
	polygenList = polygenMeshList;
	type = selectType;
...
void InteractiveTool::_selectNodes(QEvent *event, mouse_event_type event_type)
{
	...
	for (GLKPOSITION Pos = patch->GetNodeList().GetHeadPosition(); Pos != NULL;)
	{
		...
		if (geo.JugPointInsideOrNot(pointNum, xp, yp, sx, sy)) {
			...
			for (GLKPOSITION FacePos = TempNode->GetFaceList().GetHeadPosition(); FacePos!=NULL;)
			{
				QMeshFace* TempFace = (QMeshFace*)TempNode->GetFaceList().GetNext(FacePos);
				if (TempFace->GetRightTetra() == nullptr || TempFace->GetLeftTetra() == nullptr)//on the surface
				{
					if (isSelect == false&& TempNode->isHandle==false&&TempNode->isFixed==false) {
...
void InteractiveTool::_selectFixed(QEvent *event, mouse_event_type event_type)
{
	...
	PolygenMesh *polygen = (PolygenMesh*)polygenList->GetHead();
	QMeshPatch *patch = (QMeshPatch*)polygen->GetMeshList().GetHead();
	double bBox[6];  patch->ComputeBoundingBox(bBox);
		...
		if (geo.JugPointInsideOrNot(pointNum, xp, yp, sx, sy)) {
			...
			else
			{
				for (GLKPOSITION FacePos = TempNode->GetFaceList().GetHeadPosition(); FacePos != NULL;)
				{
					...
					if (TempFace->GetRightTetra() == nullptr || TempFace->GetLeftTetra() == nullptr)//on the surface
					{
						TempFace->GetNormal(fxx, fyy, fzz);


						if (isXPos == true && fxx > 0)
						{
							if (TempNode->isHandle == true) continue;
							TempNode->isFixed = true;
							printf("selected!");
						}
						if (isXNeg == true && fxx < 0)
						{
							if (TempNode->isHandle == true) continue;
							TempNode->isFixed = true;
 						}
						if (isYPos == true && fyy > 0)
						{
							if (TempNode->isHandle == true) continue;
							TempNode->isFixed = true;
 						}
						if (isYNeg == true && fyy < 0)
						{
							if (TempNode->isHandle == true) continue;
							TempNode->isFixed = true;
 						}
						if (isZPos == true && fzz > 0)
						{
							if (TempNode->isHandle == true) continue;

							TempNode->isFixed = true;
 						}
						if (isZNeg == true && fzz < 0)
						{
							if (TempNode->isHandle == true) continue;

							TempNode->isFixed = true;
 						}
						if (isXPos == false && isXNeg == false && isYPos == false && isYNeg == false && isZPos == false && isZNeg == false)
						{
							if (TempNode->isHandle == true) continue;
			...
			else
			{
				for (GLKPOSITION FacePos = TempNode->GetFaceList().GetHeadPosition(); FacePos != NULL;)
				{
					...
					if (TempFace->GetRightTetra() == nullptr || TempFace->GetLeftTetra() == nullptr)//on the surface
					{
						...
						if (isXPos==true&&fxx>0)
						{
							if (TempNode->isFixed == true) continue;
							if (handleindexinput == 0) {
								TempNode->isHandle = true;
								TempNode->handleindex = 0;
							}
							else if (handleindexinput == 1) {
								TempNode->isHandle1 = true;
								TempNode->handleindex = 1;
							}
							else if (handleindexinput == 2) {
								TempNode->isHandle2 = true;
								TempNode->handleindex = 2;
							}
							else if (handleindexinput == 3) {
								TempNode->isHandle3 = true;
								TempNode->handleindex = 3;
							}
